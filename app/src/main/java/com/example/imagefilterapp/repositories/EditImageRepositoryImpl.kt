...
class EditImageRepositoryImpl(private val context: Context) : EditImageRepository {
    ...
    override suspend fun prepareImagePreview(imageUri: Uri): Bitmap? {
        getInputSteamFromUri(imageUri).let { inputStream ->
    ...
    private fun getInputSteamFromUri(uri: Uri): InputStream? {
        return context.contentResolver.openInputStream(uri)
    }

    override suspend fun saveFilteredImage(filteredBitmap: Bitmap): Uri? {
        return try {
            val mediaStorageFilesDir = File(
                context.getExternalFilesDir(Environment.DIRECTORY_PICTURES),
                "Saved Image"
            )
            if (!mediaStorageFilesDir.exists()) {
                mediaStorageFilesDir.mkdirs()
            }
            val fileName = "IMG_${System.currentTimeMillis()}.jpg"
            val file = File(mediaStorageFilesDir, fileName)
